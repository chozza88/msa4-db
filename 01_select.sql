...
-- 데이터 값이 null이거나 not null을 확인하려면 IS NULL, IS NOT NULL로 비교해야 한다.
SELECT *
FROM employees
WHERE fire_at IS NOT NULL;

-- 출생년도가  1990년 인 사원을 조회해주세요.
SELECT *
FROM employees
WHERE 
	birth >= '1990-01-01'
...
-- 현재 받고 있는 급여 중 가장 많이 받는 급여와, 가장 적게 받는 급여를 출력해 주세요.
